...
async function filterAddressBookDataByCategoryAndSearchString(searchString: string | undefined, chainId: ChainIdWithUniversal): Promise<any> {
...
export async function getMetadataForAddressBookData(filter: GetAddressBookDataFilter) {
	const filtered = await filterAddressBookDataByCategoryAndSearchString(filter.searchString, filter.chainId)
	return {
